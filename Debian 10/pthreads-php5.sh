...
cd /
curl https://www.openssl.org/source/openssl-1.0.2u.tar.gz | tar xz && cd openssl-1.0.2u && ./config -fPIC --prefix=/home/user/build-openssl && make -j$(nproc) && make -j$(nproc) install 
apt-get update
